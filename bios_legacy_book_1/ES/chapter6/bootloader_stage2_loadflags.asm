...
.copy_setup:
    ...
    ; Configurar loadflags
    ;mov byte [esi + 0x211], 0x81 ; CAN_USE_HEAP | LOADED_HIGH
                                  ; Bit 0 (LOADED_HIGH) = 1
                                  ; Bit 7 (CAN_USE_HEAP) = 1

    ; Configurar heap_end_ptr (importante para el setup)
    ;mov word [esi + 0x224], 0xDE00  ; heap termina cerca de 0x9DE00

    ; Configurar code32_start (donde empieza el kernel protegido)
    ;mov dword [esi + 0x214], 0x100000

    ; Configurar cmd_line_ptr (ya lo tenemos de antes)
    ;mov dword [esi + 0x228], 0x0009e000
...
.copy_loop:
    ...
    ;*************************************************************************
    ; mov edi, 0x00090211
    ; mov byte [edi], 0x81     ; LOADED_HIGH (0x01) | CAN_USE_HEAP (0x80)

    ; Configurar heap_end_ptr
    ; mov edi, 0x00090224
    ; mov [edi], 0x0009de00   ; Heap termina en 0x9DE00
    ;*************************************************************************
    ; Configurar loadflags (CRÍTICO):
    ; - (ERROR que hemos cometido desde el principio).
    ;*************************************************************************
...
msg_debug_heap_end:       db '[KERNEL] [DEBUG] heap end:', 13, 10, 0
msg_kernel_ready:       db 13, 10, '[KERNEL] TODO OK => jmp', 13, 10, 0
