...
stage2_start:
    ...
    ; - realmente no lo necesitamos para lo que vamos a hacer. No lo usaremos.
    ; call vbe_controller_info
...
.setup_size_ok:
    ...
    mov byte [edi + 0x211], 0x81    ; CAN_USE_HEAP | LOADED_HIGH
    ...
    ;*************************************************************************
    ; PASO 1.b VIDMODE: screen_info
    ; **** CRÍTICO: si no configuramos esta estructura, es muy probable que
    ;      el kernel de Linux no inicialize la VGA.
    ;*************************************************************************
    mov word [edi + 0x1fa], 0xFFFF  ; Modo normal (o 0 para preservar)

    ; screen_info (offsets 0x000-0x040) - Info de pantalla
    ; orig_video_mode (offset 0x06)
    mov byte [edi + 0x06], 0x03         ; Modo texto 3 (80x25)

    ; orig_video_cols (offset 0x07)
    mov byte [edi + 0x07], 80           ; 80 columnas

    ; orig_video_lines (offset 0x0E)
    mov byte [edi + 0x0e], 25           ; 25 líneas

    ; orig_video_isVGA (offset 0x0F)
    ; - este campo es crítico.
    mov byte [edi + 0x0f], 1            ; Es VGA

    ; orig_video_points (offset 0x10) - Alto de fuente
    mov word [edi + 0x10], 16           ; 16 pixels de alto
    ;*************************************************************************
    ; //END: PASO 1.b VIDMODE: screen_info
    ;*************************************************************************

    ;*************************************************************************
    ; PASO 1.c Rellenamos campos opcionales de boot_params:
    ;*************************************************************************
    mov edi, KERNEL_BOOT_PARAMS
...
.copy_cmdline:
    ...
    mov gs, ax
    mov ss, ax
    ; Hemos alejado la pila: en 0x90000 teníamos kernel panics.
    ;mov esp, 0x90000
    mov esp, 0x7FFFF
...
kernel_cmdline:      db "console=tty0 console=ttyS0,115200 rw rdinit=/init.sh"
